...
아래 차트는 각 오케스트레이션 스킬의 단계별 실행 흐름과 하위 에이전트/스킬의 의존 관계를 보여줍니다.

```mermaid
graph LR
    subgraph WOI ["/work-on-issue  (github-workflow)"]
        WOI1["① @issue-fetcher"] --> WOI2["② @branch-creator"] --> WOI3["③ Plan Mode"] --> WOI4["④ 코드 구현<br/>+ @auto-committer"] --> WOI5["⑤ git push<br/>+ @pr-creator"]
    end

    subgraph WOP ["/work-on-pr  (github-workflow)"]
        WOP1["① @pr-fetcher"] --> WOP2["② Plan Mode"] --> WOP3["③ 코드 수정<br/>+ @auto-committer"] --> WOP4["④ gh pr comment"]
    end

    subgraph WFS ["/work-from-scratch  (github-workflow)"]
        WFS1["① @branch-creator"] --> WFS2["② Plan Mode"] --> WFS3["③ 코드 구현<br/>+ @auto-committer"] --> WFS4["④ git push<br/>+ @pr-creator"]
    end

    subgraph ANAP ["/analyze-project  (project-analyzer)"]
        ANAP1["① 병렬 분석"] --> ANA1["@structure-analyzer"] & ANA2["@docs-analyzer"] & ANA3["@settings-analyzer"]
        ANA1 & ANA2 & ANA3 --> ANA4["② @issue-creator"]
    end

    subgraph GW [git-workflow]
        BC["@branch-creator<br/>(/branch-strategy)"]
        AC["@auto-committer<br/>(/commit-message)"]
    end
