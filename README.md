...
graph LR
    ...
    subgraph WOP ["/work-on-pr  (github-workflow)"]
        WOP1["① @pr-fetcher"] --> WOP2["② 브랜치 체크아웃"] --> WOP3["③ Plan Mode"] --> WOP4["④ 코드 수정<br/>+ @auto-committer"] --> WOP5["⑤ git push<br/>+ gh pr comment"]
    end

    subgraph WFS ["/work-from-scratch  (github-workflow)"]
        WFS1["① 작업 정의"] --> WFS2["② @branch-creator"] --> WFS3["③ Plan Mode"] --> WFS4["④ 코드 구현<br/>+ @auto-committer"] --> WFS5["⑤ git push<br/>+ @pr-creator"]
    end

    subgraph ANAP ["/analyze-project  (project-analyzer)"]
        ANAP0["① 분석 범위 결정"] --> ANAP1["② 병렬 분석"]
        ANAP1 --> ANA1["@structure-analyzer"] & ANA2["@docs-analyzer"] & ANA3["@settings-analyzer"]
        ANA1 & ANA2 & ANA3 --> ANAP2["③ 종합 리포트 작성"]
        ANAP2 -. "개발자 승인 후" .-> ANA4["④ @issue-creator (선택)"]
    end
